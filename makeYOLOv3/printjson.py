...
classesFile = "./cfg.self/obj.names";
modelConfiguration = "./cfg.self/yolov3-tiny.cfg";
modelWeights = "./cfg.self/weights/yolov3-tiny_last.weights";
...
def postprocess(frame, outs, orgFrame, record, j_len):
    ...
    for i in indices:
        ...
        data_arr=json.load(open('./test2.json'))
        names = locals()
        names['dict%s'%str(j_len)]=record
        names['dict%s'%str(j_len)]['output'].append(int(classIds[i]))
        names['dict%s'%str(j_len)]['output'].append(confidences[i])
        names['dict%s'%str(j_len)]['output'].append(int(left))
        names['dict%s'%str(j_len)]['output'].append(int(top))
        names['dict%s'%str(j_len)]['output'].append(int(left + width))
        names['dict%s'%str(j_len)]['output'].append(int(top + height))
        data_arr.append(names['dict%s'%str(j_len)])
        with open('./test2.json','w') as f:
            #pdb.set_trace()
            data_arr=json.dumps(data_arr)
            f.write(data_arr)
            f.close
...
while cv2.waitKey(1) < 0:
    ...
    record_j=json.load(open('./test2.json'))
    #names = locals()
    j_len=len(record_j)+1
    jjj={'filename':args.image,'output':[]}

    postprocess(frame, outs, orgFrame, jjj, j_len)
